        ...
        <div className=" border-l-2 border-r-2">
          <div className=" m-10 mb-0">
            ...
            <div className=" flex">
              ...
              <input className=" m-4 w-full h-3 bg-gray-200 appearance-none cursor-pointer dark:bg-gray-700" type="range" min="0" max="5000000" step="100000" name="" id="num" on onChange={(event) => {
                document.getElementById("price").innerText = formatter.format(event.target.value)
                let newList = ListOfApartments.filter((Apartment) => Apartment.price <= document.getElementById("num").value
                )
          ...
          <h1 id="price" className=" text-black ml-48 mb-3">0,00 kr.</h1>
        </div>
        <div className="">
          <div className=" m-10 mb-0">
            <div className="grid grid-rows-2">
              ...
              <div className="flex">
                ...
                <input className=" m-3 w-full h-3 bg-gray-200 appearance-none cursor-pointer dark:bg-gray-700" type="range" min="0" max="300" step="20" name="" id="numH" onChange={(event) => {
                  document.getElementById("squaremeter").innerText = event.target.value + " m² "
                  let newList = ListOfApartments.filter((Apartment) => Apartment.price <= document.getElementById("num").value
                  )
                  let itemList = newList.map((item, index) => {
                    if (newList.length > 0) {
          ...
          <h1 id="squaremeter" className=" ml-48"> 0 m²</h1>
        </div>
      </div>
      <div className=" relative p-4 mt-6">
        <h1 className="text-3xl mx-5 my-5">{ListOfApartments.length.toString()} Apartments in Aalborg district</h1>
      </div>
